"""
Logistic Regression approach to MNIST Binary classification
Using Pytorch and a custom Logistic Loss
"""
...
accuracy_list = []
loss_list = []
with open('losses_logistic.txt', 'w') as f:
    for learning_rate in lr_list:
        ...
        for epoch in range(num_epochs):
            ...
            print('Accuracy of the LogisticRegression SGD model with step size %f on the test images: \t%f %%\n' % (learning_rate, 100 * (correct.float() / total)))
            accuracy_list.append(correct.float() / total)
            loss_list.append(epoch_loss)

            f.write("\n")
            f.write('Accuracy of the LogisticRegression SGD model with step size %f on the test images: \t%f %%\n' % (learning_rate, 100 * (correct.float() / total)))
